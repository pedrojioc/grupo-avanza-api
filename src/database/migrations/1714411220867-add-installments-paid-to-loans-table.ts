...
export class AddInstallmentsPaidToLoansTable1714411220867 implements MigrationInterface {
  name = 'AddInstallmentsPaidToLoansTable1714411220867'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`loans\` ADD \`installments_paid\` int NOT NULL DEFAULT '0'`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`loans\` DROP COLUMN \`installments_paid\``)
  }
}
